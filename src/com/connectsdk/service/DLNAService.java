...
public class DLNAService extends DeviceService implements MediaControl,
		MediaPlayer {
	...
	public DLNAService(ServiceDescription serviceDescription,
	...
	public void displayMedia(final String url, final String mimeType,
		...
		if (mediaType == null || mediaType.length() == 0 || mediaFormat == null
				|| mediaFormat.length() == 0) {
			Util.postError(
					listener,
					new ServiceCommandError(
							0,
							"You must provide a valid mimeType (audio/*,  video/*, etc)",
							null));
			return;
		}
		...
		ResponseListener<Object> responseListener = new ResponseListener<Object>() {
			...
			public void onSuccess(Object response) {
				...
				ResponseListener<Object> playResponseListener = new ResponseListener<Object>() {
					...
					public void onSuccess(Object response) {
						...
						Util.postSuccess(listener, new MediaLaunchObject(
								launchSession, DLNAService.this));
					}
				...
				ServiceCommand<ResponseListener<Object>> request = new ServiceCommand<ResponseListener<Object>>(
						DLNAService.this, method, payload, playResponseListener);
				request.send();
			}
		...
		JSONObject httpMessage = getSetAVTransportURIBody(method, instanceId,
				url, mMimeType, title);

		ServiceCommand<ResponseListener<Object>> request = new ServiceCommand<ResponseListener<Object>>(
				DLNAService.this, method, httpMessage, responseListener);
		request.send();
	}

	public void displayMedia(final MediaInfo mediaInfo,
			final LaunchListener listener) {
	...
	public void playMedia(final String url, final String mimeType,
			final String title, final String description, final String iconSrc,
			final boolean shouldLoop, final LaunchListener listener) {
		displayMedia(url, mimeType, title, description, iconSrc, listener);
		...
		// String mediaFormat = mediaElements[1];
		//
		// if (mediaType == null || mediaType.length() == 0 || mediaFormat ==
		// null || mediaFormat.length() == 0) {
		// Util.postError(listener, new ServiceCommandError(0,
		// "You must provide a valid mimeType (audio/*,  video/*, etc)", null));
		// return;
		// }
		//
		// mediaFormat = "mp3".equals(mediaFormat) ? "mpeg" : mediaFormat;
		...
		// }
		//
		// ResponseListener<Object> playResponseListener = new
		...
		// };
		//
		// ServiceCommand<ResponseListener<Object>> request = new
		// ServiceCommand<ResponseListener<Object>>(DLNAService.this, method,
		// obj, playResponseListener);
		// request.send();
		// }
		// });
	}
	...
	public void seek(long position, ResponseListener<Object> listener) {
		...
		String time = String.format(Locale.US, "%02d:%02d:%02d", hour, minute,
				second);
	...
	private void getPositionInfo(final PositionInfoListener listener) {
		...
		ServiceCommand<ResponseListener<Object>> request = new ServiceCommand<ResponseListener<Object>>(
				this, method, payload, responseListener);
		request.send();
	}
	...
	private JSONObject getSetAVTransportURIBody(String method,
			String instanceId, String mediaURL, String mime, String title) {
	...
	private String getMetadata(String mediaURL, String mime, String title) {
		...
		sb.append("&lt;item id=&quot;" + id + "&quot; parentID=&quot;"
				+ parentID + "&quot; restricted=&quot;" + restricted
				+ "&quot;&gt;");
		...
		sb.append("&lt;res protocolInfo=&quot;http-get:*:" + mime
				+ ":DLNA.ORG_OP=01&quot;&gt;" + mediaURL + "&lt;/res&gt;");
	...
	public void sendCommand(final ServiceCommand<?> mCommand) {
		Util.runInBackground(new Runnable() {
			...
			public void run() {
				...
				HttpPost request = HttpMessage.getDLNAHttpPost(controlURL,
						command.getTarget());
	...
	public LaunchSession decodeLaunchSession(String type, JSONObject sessionObj)
		...
		if (type == "dlna") {
			LaunchSession launchSession = LaunchSession
					.launchSessionFromJSONObject(sessionObj);
